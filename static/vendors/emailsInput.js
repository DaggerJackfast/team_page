define(function () {
     ...
     */
    function checkAndFixInputPlaceHolder(options) {
        const hasEmails = listOfValidEmails.length > 0;
        const inputBox = document.querySelector('input.email-input');
        if (hasEmails) {
            inputBox.placeholder = options.morePlaceholder ?? 'add more people...';
        } else {
            inputBox.placeholder = options.morePlaceholder ?? 'add people...';
        }
    }
    ...
    function addEmailToList(emailsContainer, email, options) {
        ...
        removeBtn.addEventListener('click', function () {
            ...
            checkAndFixInputPlaceHolder(options);
        });
